...
1. Делаем в GitHub новый репозиторий.
2. Создаем на компьютере папку для локального хранения репозитория.
3. Создаем с помощью git init новый репозиторий из созданной папки.
4. Связываем с помощью git remote add origin локальный репозиторий (п.3) и репозиторий GitHub (п.1).
5. Переименовываем ветку на локальном репозитории в "main" с помощью git branch -M main.
6. Вносим изменения в содержимое проекта локально.
7. С помощью git push -u origin main отправляем изменения в свой удаленный репозиторий на GitHub.
8. Используем git pull, чтобы скачать последние изменения с удаленного репозитория на локальный.
9. Используем git clone, чтобы полностью клонировать удаленный репозиторий на локальный компьютер.

## Чтобы предлагать свои изменения другим пользователям:
1. Используем fork интересующего удаленного репозитория, чтобы вносить свои изменения.
2. Клонируем с помощью git clone удаленный репозиторий на наш локальный компьютер.
3. Создаём новую ветку в нашем локальном репозитории.
4. Работаем с содержимым, вносим изменения.
5. Отправляем изменения в свой удаленный репозиторий на GitHub.
6. В GitHub создаем pull request с последними изменениями.

# Это последнее задание по курсу "Введение в контроль версий". Большое спасибо Ильнару за лекции и Елене за семинары! =)
